...
- `tests/fixtures/gdpr_data_samples_main/generated_format_challenges.json` preserves generated high-difficulty multilingual format cases, including header-row CSV travel identity data, UTF-16 text, XML, RTF, newline-delimited JSON, OpenDocument, EML email, ZIP archive, image OCR, and PDF OCR cases with OCR-degraded label separators, without mutating Drive originals.
- `tests/fixtures/gdpr_data_samples_main/core_negative_cases.json` preserves runnable no-sensitive-info cases for clean enterprise budgets, SKUs, release configs, Markdown tables, CSV inventory rows, OCR receipts, PDF OCR release notes, table amounts, and operations logs so false positives are measured rather than assumed absent.
