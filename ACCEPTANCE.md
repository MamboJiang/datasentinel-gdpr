...
- `tests/fixtures/gdpr_data_samples_main/core_engine_quality_report.json` records repeatable type-level precision, recall, F1, false-positive counts, and false-negative counts across the multilingual and generated-format oracle cases without raw source text or raw detected values.
...
- A reproducible quality report script evaluates multilingual source-text and generated-format oracle cases, records aggregate type-level precision/recall/F1 plus per-case false-positive and false-negative signal types, and stores only signal-type metadata.
