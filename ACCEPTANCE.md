...
- Opening a stale finding detail route whose `findingId` is no longer present in the current scan/workspace state recovers to the highest-priority current finding, or to the findings list when there are no current findings, instead of rendering seeded or obsolete evidence.
...
- TXT, Markdown non-table text, transcript, RTF, PDF page text, and OCR text-position anchors include source-local character offsets plus line/column metadata when scan-time line starts are available, without exposing raw source text.
...
- PNG, JPG/JPEG, TIFF, BMP, and WEBP image files can be scanned through host-local Tesseract OCR when `LAWDIT_OCR_MODE=local` and the host binary is available, without storing raw images or raw OCR text; non-PNG image inputs can fall back to a temporary PNG-normalized OCR candidate and Drive/direct-link download acceptance includes the declared image MIME types.
...
- `tests/fixtures/gdpr_data_samples_main/core_multilingual_cases.json` preserves runnable multilingual source-text cases for deterministic scanner tests, including travel, driver-license, and signature labels in non-English forms.
- `tests/fixtures/gdpr_data_samples_main/generated_format_challenges.json` preserves generated high-difficulty multilingual format cases, including header-row CSV and TSV travel identity data, LOG key-value identity data, UTF-16 text, XML, RTF, newline-delimited JSON, OpenDocument, EML email, ZIP archive, PNG/BMP/TIFF/WEBP image OCR, and PDF OCR cases with OCR-degraded label separators, without mutating Drive originals.
- `tests/fixtures/gdpr_data_samples_main/core_negative_cases.json` preserves runnable no-sensitive-info cases for clean enterprise budgets, SKUs, release configs, Markdown tables, CSV inventory rows, OCR receipts, PDF OCR release notes, table amounts, signature-metadata logs, and operations logs so false positives are measured rather than assumed absent.
...
- `tests/fixtures/gdpr_data_samples_main/engine_hardening_validation_report.json` records final-hardening validation evidence for local tests, prelaunch backend delta proof, supported-format UI proof, agent-us deployment, OCR runtime capability, and real OCR smoke tests without raw source values or provider tokens.
...
- Deterministic signal detection recognizes common Chinese, EU-language, Japanese, Korean, and Arabic form labels for names, phone numbers, email, birth dates, national identifiers, tax IDs, passports, driver licenses, travel records, signatures, addresses, bank accounts, health data, compensation, and credentials.
- Deterministic signal detection splits inline static-text, LOG key-value, stacked OCR/PDF form layouts, and OCR-normalized label/value sequences such as `姓名：... 电话：... 地址：...`, `Name\n...`, `姓名\n...`, `Reisender\n...`, `passport_number=... booking_reference=...`, punctuation-missing CJK/Kana/Hangul/Arabic labels such as `姓名 ... 电话 ...`, separatorless CJK/Kana/Hangul OCR text such as `姓名王芳电话...`, dash-delimited labels such as `Name - ...`, travel identity labels such as `Passenger`, `Booking Reference`, `Führerschein`, `Buchungsnummer`, `旅客`, `驾驶证号`, and `预订编号`, and OCR-degraded passport labels such as `Passport EN...` when the value has a plausible identifier shape, into separate redacted signals with source-local `textPosition` or table-cell anchors, while suppressing overlapping same-type regex duplicates, weaker overlapping regex classifications when stronger contextual labels already explain the value, signature-metadata fields such as `signature_algorithm`, stacked product/release metadata such as `Release Name\nPhoenix`, and context-free corporate budget/SKU/numeric-log/receipt patterns that are not personal data.
...
- Local OCR scores bounded language-profile outputs and can continue past a low-signal non-empty OCR result so a stronger multilingual profile can preserve labels such as `姓名`/`地址` and passport-shaped identifiers for redacted detection.
...
- Color-overlay OCR preprocessing crops and scales foreground text masks before Tesseract to reduce full-page background noise in image and PDF OCR scans.
...
- Automated backend tests cover raw corpus checksum and Drive coverage, real PDF corpus text-layer extraction, mixed PDF text-layer-plus-OCR extraction, corpus fixture readability, OCR capability reporting, multilingual detection redaction, stacked OCR/PDF form-label detection, travel/driver-license/signature detection, LOG key-value detection, signature-metadata false-positive suppression, deterministic quality report freshness/privacy, inline/OCR-normalized label-sequence detection, OCR-degraded passport-label detection, clean receipt/SKU amount suppression, stacked product/release metadata false-positive suppression, separatorless CJK OCR word-join detection, bounded signal detection for large text streams, UTF-16 text-like decoding, generated multiformat challenge scanning, structured text-like file scanning, TSV/CSV header-row table-cell extraction, Markdown table header extraction, RTF rich-text scanning, OpenDocument text extraction, legacy Office conversion, EML email extraction, ZIP archive member extraction, source-data evidence-anchor redaction, source-review context-window redaction, CSV header-row and delimiter-sniffed table-cell anchor redaction, CSV/TSV/XLSX/ODS table-cell anchor redaction, DOCX/PPTX/ODT/ODP/EML/HTML/XML/JSONL/NDJSON structure-path anchor redaction, PDF text-layer page-region anchor redaction, image OCR region anchor redaction, PDF OCR page-image region anchor redaction, scan assembly from multilingual cases, PDF OCR fallback, and missing-tool OCR-deferred behavior.
...
- Prelaunch backend delta scans preserve private per-source baseline fingerprints after full scan completion, reject missing or mismatched baselines, and assemble findings only for files whose private fingerprint is new or modified.
