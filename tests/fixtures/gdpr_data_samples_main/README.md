...
- `generated_format_challenges.json` contains synthetic multilingual CSV, header-row travel identity CSV, HTML, Office, image OCR, PDF OCR, OCR-degraded label, and transcript cases generated for high-difficulty format tests without mutating Drive originals. Raw video frame OCR and legacy Office conversion use temporary files generated during runtime tests; the video, legacy Office bytes, converted text, and OCR text are not persisted here.
- `core_negative_cases.json` contains synthetic no-sensitive-info enterprise text, CSV, JSON, Markdown, log, image OCR, and PDF OCR cases used to measure false-positive behavior for budgets, SKUs, release configs, procurement text, clean receipts, release notes, table amounts, and operations numbers.
