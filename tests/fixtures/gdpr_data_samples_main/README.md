...
- `generated_format_challenges.json` contains synthetic multilingual CSV, HTML, Office, image OCR, PDF OCR, OCR-degraded label, and transcript cases generated for high-difficulty format tests without mutating Drive originals. Raw video frame OCR and legacy Office conversion use temporary files generated during runtime tests; the video, legacy Office bytes, converted text, and OCR text are not persisted here.
...
- `core_engine_quality_report.json` records deterministic oracle quality for multilingual, generated-format, and no-sensitive-info cases as type precision, recall, F1, false-positive type counts, and false-negative type counts only. It must not contain raw source text or raw detected values.
