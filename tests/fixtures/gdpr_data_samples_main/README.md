...
- `generated_format_challenges.json` contains synthetic multilingual CSV, header-row travel identity CSV/TSV, LOG key-value travel identity, HTML, Office, image OCR, PDF OCR, OCR-degraded label, and transcript cases generated for high-difficulty format tests without mutating Drive originals. Raw video frame OCR and legacy Office conversion use temporary files generated during runtime tests; the video, legacy Office bytes, converted text, and OCR text are not persisted here.
