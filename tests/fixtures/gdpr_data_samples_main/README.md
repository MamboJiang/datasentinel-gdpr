...
- `core_multilingual_cases.json` contains synthetic multilingual source-text cases used by automated deterministic scanner tests, including travel, driver-license, and signature labels.
- `generated_format_challenges.json` contains synthetic multilingual CSV, header-row travel identity CSV/TSV, LOG key-value travel identity, HTML, Office, PNG/BMP/TIFF/WEBP image OCR, PDF OCR, OCR-degraded label, and transcript cases generated for high-difficulty format tests without mutating Drive originals. Raw video frame OCR and legacy Office conversion use temporary files generated during runtime tests; the video, legacy Office bytes, converted text, and OCR text are not persisted here.
- `core_negative_cases.json` contains synthetic no-sensitive-info enterprise text, CSV, JSON, Markdown, log, image OCR, and PDF OCR cases used to measure false-positive behavior for budgets, SKUs, release configs, procurement text, clean receipts, release notes, table amounts, signature metadata, and operations numbers.
...
- `core_engine_quality_report.json` records deterministic oracle quality for multilingual, generated-format, and no-sensitive-info cases as type precision, recall, F1, false-positive type counts, and false-negative type counts only, including travel-record, driver-license, account-handle, signature, TSV, LOG key-value, and signature-metadata negative coverage. It must not contain raw source text or raw detected values.
