...
`core_multilingual_cases.json` is the deterministic local test companion for the core engine. It preserves multilingual source-text cases for Chinese, French, Spanish, German, Italian, Portuguese, Dutch, Polish, Japanese, Korean, and Arabic labels, including travel, driver-license, signature labels, and stacked OCR/PDF form-label layouts where values appear on following lines, so automated tests can run without a live Google Drive token.

`generated_format_challenges.json` is the generated high-difficulty companion corpus. It covers multilingual CSV, header-row travel identity CSV/TSV, LOG key-value travel identity lines, HTML, XML, JSONL, UTF-16 text, RTF, DOCX, XLSX, PPTX, ODT, ODS, ODP, EML, ZIP, PNG/BMP/TIFF/WEBP image OCR, PDF OCR, OCR-degraded passport-label output, and VTT transcript inputs without modifying Drive originals. Raw video frame OCR and legacy Office conversion coverage are generated at test runtime as temporary files and recorded only through redacted validation evidence.

`core_negative_cases.json` is the synthetic clean-corpus companion. It covers enterprise budgets, product SKUs, release configs, stacked release metadata, Markdown budget tables, CSV inventory rows, Chinese and German procurement text, OCR clean receipts, PDF OCR release notes, table amounts, signature-metadata logs, and operations logs that should not create GDPR-sensitive findings.

`core_engine_quality_report.json` is the redacted deterministic quality record. It evaluates `core_multilingual_cases.json`, `generated_format_challenges.json`, and `core_negative_cases.json` as type-level oracle cases, including travel-record, driver-license, account-handle, signature, stacked OCR/PDF form-label, TSV table, LOG key-value, and signature-metadata negative coverage, and records precision, recall, F1, false-positive type counts, and false-negative type counts without raw source text, raw detected values, source bodies, provider tokens, Drive URLs, page images, or private paths. Regenerate it with `tests/core_engine_quality_report.py` after detector, parser, OCR-normalization, or corpus changes.

`engine_hardening_validation_report.json` is the redacted final-hardening evidence record. It records the read-only live Drive folder check, local and agent-us validation commands, prelaunch backend delta proof, static-format deepening for header-row CSV, Markdown tables, source-review context windows, and mixed PDFs, supported-format UI proof, deployed release identifiers, OCR/converter runtime capability, and real OCR/image/PDF/video-frame/legacy-Office smoke-test summaries without raw source values or provider tokens.
...
`live_drive_scan_report_agent_us.json` is the redacted live server-scan record. It proves the deployed `agent-us` runtime can scan the `GDPR-data-samples-main` folder through an account-level Google Drive binding, record aggregate extraction/finding/source-preview metrics including `pdf_mixed` OCR evidence, and keep provider tokens, Drive URLs, raw source bodies, raw detected values, page images, and private absolute paths out of the report. Regenerate it with `scripts/generate_live_drive_scan_report.py` on the deployed host after Drive, extraction, OCR, or corpus changes.
...
- Keep policy-pack version, dataset hash, precision/recall/F1, false-positive counts, and false-negative counts in evaluation output without raw source values.
