...
`generated_format_challenges.json` is the generated high-difficulty companion corpus. It covers multilingual CSV, header-row travel identity CSV/TSV, LOG key-value travel identity lines, HTML, XML, JSONL, UTF-16 text, RTF, DOCX, XLSX, PPTX, ODT, ODS, ODP, EML, ZIP, image OCR, PDF OCR, OCR-degraded passport-label output, and VTT transcript inputs without modifying Drive originals. Raw video frame OCR and legacy Office conversion coverage are generated at test runtime as temporary files and recorded only through redacted validation evidence.
