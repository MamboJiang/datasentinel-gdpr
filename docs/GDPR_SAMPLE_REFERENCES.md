...
`core_multilingual_cases.json` is the deterministic local test companion for the core engine. It preserves multilingual source-text cases for Chinese, French, Spanish, German, Italian, Portuguese, Dutch, Polish, Japanese, Korean, and Arabic labels, including travel, driver-license, and signature labels, so automated tests can run without a live Google Drive token.
