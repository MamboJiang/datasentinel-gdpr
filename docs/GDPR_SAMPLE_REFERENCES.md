...
`core_engine_quality_report.json` is the redacted deterministic quality record. It evaluates `core_multilingual_cases.json` and `generated_format_challenges.json` as type-level oracle cases and records precision, recall, F1, false-positive type counts, and false-negative type counts without raw source text, raw detected values, source bodies, provider tokens, Drive URLs, page images, or private paths. Regenerate it with `tests/core_engine_quality_report.py` after detector, parser, OCR-normalization, or corpus changes.
