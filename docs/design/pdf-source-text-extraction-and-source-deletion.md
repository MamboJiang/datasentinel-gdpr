...
| Delete source registration only | Remove the DataSentinel source metadata row and leave external files untouched. | Accepted because it corrects wrong setup state without mutating user files. |
...
The source API exposes `DELETE /api/sources/{sourceId}`. The route deletes only the DataSentinel source registration from the active store and returns the deleted source envelope. Missing source ids return `application/problem+json` with `404`.
...
- Source deletion removes the source registration row from DataSentinel state.
...
- A source registration can be deleted from DataSentinel and is absent from the next source list.
