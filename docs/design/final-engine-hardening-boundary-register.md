...
| BND-003 | Critical | Real OCR depends on server-installed `tesseract`, `pdftoppm`, language packs, and image preprocessing. | Image-only PDFs, mixed-layer PDFs, and color text overlays are core hard cases; mocked tests are not enough for final proof. | `agent-us` has Tesseract 5.3.4, `pdftoppm`, Pillow, and OCR languages `eng+chi_sim+deu+fra+spa+por+ita+nld+pol+jpn+kor+ara`; `/api/health` reports OCR `local_available`; OCR now uses bounded multilingual profiles and continues after one profile/candidate timeout. Real deployed extraction of the 6 MB V4 PDF returns `pdf_mixed` with redacted person, address, passport, phone, location, and organization signal types plus `pdf_ocr` anchors. | Keep host OCR packages, Pillow, and `LAWDIT_OCR_MODE=local`; rerun the smoke report after OCR package, language, or preprocessing changes. | `tests.test_pdf_region_anchors`, `tests.test_ocr_region_anchors`, agent-us direct PDF smoke, full server suite. |
...
| BND-010 | Medium | Performance and quality boundaries need repeatable mixed-corpus profiling. | "High performance, stable, accurate" needs measured throughput, memory, precision, recall, false positives, and false negatives. | `core_engine_performance_report.json` records local generated challenge, raw corpus, and oversized text benchmark metrics with separate text and complex-document byte limits. `core_engine_performance_report_agent_us.json` records the deployed `agent-us` run with local OCR and color-overlay preprocessing available: 35 files processed, 0 unsupported, 0 OCR-deferred, 0 model calls, and `pdf_mixed` evidence for the large V4 PDF. `core_engine_quality_report.json` records deterministic oracle quality for 37 multilingual/generated-format/no-sensitive-info cases with type precision 1.0, recall 1.0, 0 false-positive types, and 0 false-negative types without raw values. | Keep `tests/core_engine_performance_benchmark.py` and `tests/core_engine_quality_report.py`; refresh reports on parser, detector, OCR, corpus, or deployment changes. | `tests.test_core_engine_performance_report`, `tests.test_core_engine_quality_report`, plus local and agent-us benchmark runs. |
