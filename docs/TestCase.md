...
| CORE-011A1 | Scan standard header-row CSV files | Header-row CSV files such as `Name,Date of Birth,Address` and `Passenger,Driver License,Username,Signature,Booking Reference` create label-context fragments for each data cell, detect label-only values such as names, birth dates, addresses, driver licenses, account handles, signatures, and travel records, and attach redacted `tableCell` anchors to the data row and column. |
