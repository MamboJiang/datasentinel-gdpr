...
| --- | --- | --- |
| SUBMIT-001 | Review GitHub visibility and homepage | The remote repository is public and its homepage points to `https://founder-force.uk/`. |
| SUBMIT-002 | Review README presentation | The README includes the product thesis, live demo URL, user guide URL, demo walkthrough, local development steps, validation commands, and safety boundaries. |
| SUBMIT-003 | Review public repository scope | The public tree does not contain AI-agent operating instructions, internal AI delivery workflow notes, semifinal script drafts, QR scratch assets, secrets, generated build artifacts, runtime data, or local environment files. |
| SUBMIT-004 | Review repository language | Developer-facing docs, engineering instructions, code comments, contracts, mocks, fixtures, issues, pull requests, and commit messages are written in English; multilingual source strings appear only as product test fixtures or redacted-detection examples. |
| SUBMIT-005 | Review collaborators and history | Requested teammates are invited as collaborators or already have their contributions preserved in repository history. |
...
| CONTRACT-006 | Review public contract boundaries | Public contract materials expose API shape, tolerance rules, governance concepts, and safety boundaries without relying on hidden delivery instructions. |
...
## Public Upload Analysis Entry Checks

| ID | Scenario | Expected Result |
| --- | --- | --- |
| UPLOAD-001 | Request website analysis capacity | `/api/public-analysis/capacity` returns max active analyses, active analyses, available slots, waiting users, queue mode, per-session active state, and the 10 MB file-size limit. |
...
| UPLOAD-004 | Submit a second file for the same active website analysis session | The backend returns `409 application/problem+json` with `userHasActiveAnalysis = true`. |
...
| UPLOAD-006 | Review safety boundaries | Website analysis output keeps raw sensitive values out of public output and prohibits legal advice, full GDPR-compliance claims, automatic deletion, and production tenant integration. |
| UPLOAD-007 | Use the homepage website analysis UI | The root homepage shows a prominent upload entry, live capacity data, one-file/10 MB limits, start-over behavior, and redacted results separate from the full Workspace console. |
...
| SRCIN-009B | Trigger a frontend scan while the API is unreachable and local mocks are disabled | The frontend marks the project server unavailable, shows that local mock fallback is disabled for the deployment, and does not run local scan, review, or Workspace mutation workflows. |
...
| SRCIN-015 | Scan an image file with host OCR available | The scanner extracts OCR text during scan execution, creates only redacted findings, reports `image_ocr` and hard difficulty, and keeps model calls at zero. PNG, JPG/JPEG, TIFF, BMP, and WEBP are accepted by suffix or image MIME type. |
...
| CORE-002 | Scan multilingual deterministic cases | Chinese, French, Spanish, German, Italian, Portuguese, Dutch, Polish, Japanese, Korean, and Arabic form labels produce redacted signals for names, phone numbers, email, birth dates, IDs, tax IDs, passports, driver licenses, travel records, signatures, addresses, bank accounts, compensation, health, and credentials. |
...
| CORE-002C | Scan OCR-degraded Latin identity labels | OCR output such as `Passport EN...` where the separator is dropped still produces a redacted passport signal when the value has a plausible identifier shape, while ordinary sentences such as `Passport applications...` produce no finding. |
| CORE-002D | Scan travel identity and signature labels | German and Chinese labels such as `Reisender`, `Führerschein`, `Buchungsnummer`, `Unterschrift`, `旅客`, `驾驶证号`, `预订编号`, and `签名` produce redacted person, driver-license, travel-record, and signature signals, while technical metadata such as `signature_algorithm` produces no person or signature signal. |
| CORE-002E | Scan LOG key-value identity labels | Log lines such as `user=... passport_number=... booking_reference=... signature="..."` produce redacted account-handle, passport, travel-record, and signature signals with text-position anchors, while `signature_algorithm=... passport_policy=... user="Human Name"` produces no finding. |
| CORE-002F | Scan stacked OCR/PDF form labels | OCR/PDF extracted text where known labels such as `Name`, `姓名`, `护照号`, `Reisender`, `Führerschein`, and `Buchungsnummer` appear on one line and values appear on following nearby lines produces redacted signals anchored to the value line, while stacked product metadata such as `Release Name` followed by a codename produces no person finding. |
...
| CORE-003A | Generate deterministic core-engine quality report | The quality report evaluates multilingual, generated-format, and no-sensitive-info oracle cases, records type-level precision, recall, F1, false-positive type counts, and false-negative type counts, and does not persist raw source text or raw detected values. |
| CORE-003B | Scan no-sensitive-info enterprise samples | Clean budget, SKU, release-config, Markdown table, CSV inventory, procurement, image OCR receipt, PDF OCR release-note, table-amount, and operations-log samples produce zero GDPR-sensitive signal types while still participating in quality metrics. |
...
| CORE-010 | Review deterministic text-position anchors | Detected label and regex signals include `evidenceAnchor.selector.type = textPosition`, stable start/end offsets, source-local line/column metadata when available, a redacted fallback label, and no raw matched value in serialized public payloads. |
| CORE-011 | Scan structured text-like files | CSV, TSV, LOG, XML, JSON, JSONL/NDJSON, HTML/HTM, suffixless Unicode text, and UTF-16 text-like files with multilingual labels produce redacted findings, easy difficulty counts, and source-data evidence anchors without leaking raw values. Binary suffixless files remain unsupported. |
...
| CORE-011A1 | Scan standard header-row CSV and TSV files | Header-row files such as `Name,Date of Birth,Address`, `Passenger,Driver License,Username,Signature,Booking Reference`, and multilingual TSV travel identity rows create label-context fragments for each data cell, detect label-only values such as names, birth dates, addresses, driver licenses, account handles, signatures, passports, phones, and travel records, and attach redacted `tableCell` anchors to the data row and column. |
