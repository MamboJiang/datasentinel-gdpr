...
The approved source-input implementation is a prelaunch boundary for direct HTTPS file links and user-selected Google Drive files or folders. It is documented in `docs/design/google-drive-source-integration.md` and `docs/design/google-drive-account-binding.md`.

Technical constraints:

- `remote_file_link` uses `config.url`, requires HTTPS, rejects embedded credentials, rejects private-address hosts, rejects Google Drive or Google Docs share pages, and supports only BOM/charset-aware Unicode text-like content, XML and JSON/JSONL/NDJSON structure extraction, bounded RTF text extraction, RFC 5322/MIME email text extraction, bounded ZIP archive member extraction, PDF text layers, bounded local PDF OCR fallback when host tooling is available, Office Open XML content, OpenDocument content, supported image files, supported transcript files, or bounded raw video media. Extractable text-like files must stay within the prelaunch size limit; raw video media must stay within the bounded video size limit and is reported as hard/OCR-deferred when FFmpeg or local OCR is unavailable.
- `google_drive_selection` uses Google Picker in the browser to collect selected item metadata. Picker can use browser OAuth or a short-lived access token minted from a signed-in account's server-side Drive binding, and scans can run with either a short-lived Picker access token or the server-side Drive binding.
...
- The backend exposes Drive binding routes under `/api/integrations/google-drive/binding` so Account settings can connect, inspect, change, or disconnect a personal binding without returning refresh tokens, and exposes `POST /api/integrations/google-drive/picker-token` to mint browser-use-only short-lived Picker access tokens for bound accounts.
- Google Drive scans receive `authorization.googleDriveAccessToken` only in the scan request or obtain a refreshed short-lived token from the account binding. Per-scan tokens are not persisted; binding refresh tokens stay server-side in the local prelaunch account store. Frontend status presentation may treat a Drive source as connected while a Picker token is present in browser memory or while the account binding is connected.
...
- Production NER, parser services, Microsoft Graph, OAuth, directory, notification, ticketing, and deletion integrations are not added in this slice; signal detection remains deterministic and adds only bounded multilingual label rules.
